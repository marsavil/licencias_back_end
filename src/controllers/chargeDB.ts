...
const charge = {
  chargeDB: async function () {
    ...
    for (let i = 0; i < admins.length; i++) {
      const adminDB = await Admin.find({ id: admins[i].id });
      if (!adminDB.length) {
    ...
    for (let i = 0; i < medicos.length; i++) {
      ...
      if (!medicoDB.length) {
        ...
        const adminDB = await Admin.find({ dni: "00000000" });
        await Medico.create({
          id: medicos[i].id,
          name: medicos[i].name,
    ...
    for (let i = 0; i < employees.length; i++) {
      ...
      if (!sectorDB) console.log(`No existe el sector ${employees[i].sector}. Debe ser creado previamente.`)
      let hashed = await bcrypt.hash(employees[i].password, ROUNDS);
      let adminDB = await Admin.find({ dni: "00000000"})
    ...
    try {
      ...
      for (let i = 0; i < employeesDB.length; i++) {
        ...
        if (sectorDB.employees.includes(employeesDB[i]._id)){
          console.log(`El empleado con el id ${employeesDB[i]._id} ya forma parte del sector ${sectorDB.name}`)
          return
        } else {
        }
        sectorDB ? sectorDB.employees.push(employeesDB[i]._id) : 'sector not found';
        sectorDB.save();
